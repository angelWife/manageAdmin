<template>
  <div id="sginContainer" v-show="diaologVO.show">
    <div class="headBtn">
      ...
      <!-- URL地址打开文档，WebOpen2的参数是文件在服务的相对路径-->
        <button type="primary" @click="LoadServlet()">打开文件</button>
      <button type="primary" @click="saveMyFile()">确认完成签章</button>
...
export default {
  ...
  data() {
    return {
        filePath: "http://192.168.31.86:8080/files/test.doc",
        ServerUrl:'http://192.168.31.86:7778/OfficeServer',
        fileName:'test.doc',
  ...
  methods: {
    ...
    Load() {
      try {
        WebOfficeObj.ServerUrl = this.filePath;
    ...
    LoadURL() {
      ...
      try {
        ...
        WebOfficeObj.FileName = "test1.doc";
        let downloadLink = "";
        this.SetGraySkin(); //设置控件皮肤
        if (WebOfficeObj.WebOpen2(downloadLink)) {
    ...
    LoadServlet() {
      ...
      try {
        ...
        let downloadLink = self.filePath;
    ...
    openMyFile() {
      ...
      try {
        this.OnUnLoad();
        // WebOfficeObj.ServerUrl = ServerUrl; // 用来保存文件的Server
        let downloadLink = this.ServerUrl;
         WebOfficeObj.FileName = this.fileName;
        // WebOfficeObj.ShowMenu = 0;
        // WebOfficeObj.ShowToolBar = 0;
        this.SetGraySkin(); //设置控件皮肤
    ...
    saveMyFile() {
      ...
      try {
        ...
        // this.OnUnLoad();
        WebOfficeObj.WebUrl = this.ServerUrl;
        WebOfficeObj.TOKEN = this.token
        WebOfficeObj.FileName = this.fileName;
        WebOfficeObj.BizData={type:1,name:'ceshi'};
        // this.SetGraySkin(); //设置控件皮肤
        // if (WebOfficeObj.WebGetFile()) {
            let res = WebOfficeObj.WebPutFile()
    ...
    getFileInfo() {
      WebOfficeObj.WebGetFile("static", "Document");
    },
    SetGraySkin() {
...
#sginContainer .footBtn {
  position: fixed;
  bottom: 0;
  right: 0;
  padding: 20px;
}
#sginContainer .headBtn{
    text-align: right;
    padding:10px 20px;
}
#sginContainer .headBtn button{
    background: #409eff;
    border-radius:0.3em;
    padding:5px 15px;
    border:0;
