window.console = window.console || (function () {
	let c = {}; c.log = c.warn = c.debug = c.info = c.error = c.time = c.dir = c.profile = c.clear = c.exception = c.trace = c.assert = function () { };
	return c;
})();
...
function WebOffice2015() {
	...
	this.GetSingleFileName = function (downloadLink) {
		...
		let index1 = filename.lastIndexOf("/");
		let index2 = filename.length;
		let postf = filename.substring(index1 + 1, index2);//后缀名
		...
		if (httpclient.Open(this.HttpMethod.Get, this.ServerUrl + downloadLink, false)) {
			if (httpclient.Send()) {
				if (this.hiddenSaveLocal(httpclient, this, false, false)) {
					let mSaveResult = this.WebOpenLocalFile(this.DownloadedFileTempPathName);
					if (mSaveResult == 0) { // 打开本地磁盘文件
						this.getOfficeVersion();// 打开文档后，判断当前office版本
						return true;
					} else if (mSaveResult == 1) {
						let windows = window.confirm("可能当前授权码错误，请确认iWebOffice2015.js的授权是否正确（或乱码）"
						...
						let windows = window.confirm("没有找到文档，请确认WebOpenLocalFile打开文档的路径是否正确"
							+ "\r\r单击“确定”关闭。单击“取消”继续。");
						...
						let windows = window.confirm("没有权限导致文档打开失败，请用管理员身份运行浏览器后重试"
							+ "\r\r单击“确定”关闭。单击“取消”继续。");
						...
						let windows = window.confirm("文件可能损坏，请确定服务器文档是否已经损坏"
							+ "\r\r单击“确定”关闭。单击“取消”继续。");
						...
						let windows = window.confirm("未安装Office或者注册表有损坏"
							+ "\r\r单击“确定”关闭。单击“取消”继续。");
						...
						let windows = window.confirm("文件被占用，请结束Office进程后重试"
							+ "\r\r单击“确定”关闭。单击“取消”继续。");
						...
						let windows = window.confirm("打开文档时未知错误！错误码为： "
							+ mSaveResult
							+ "\r\r单击“确定”关闭。单击“取消”继续。");
	...
	this.WebOpenBase64 = function () {
		const jsWebOffice = this;
		const officeobj = this.obj;
		const filename = this.FileName;
		const tmpDownPath = this.DownFilePath();

		const send = "OPTION=LOADFILE&FILENAME=" + this.FileName + "&FILETYPE=" + this.FileType + "&RECORDID=" + this.RecordID + "&EDITTYPE=" + this.EditType;
	...
	this.WebSave2 = function (callback) {
		...
		const mSaveResult = this.WebSaveLocalFile(fileUpPathName);
		if (!(mSaveResult == 0)) {
		...
		const base64file = officeobj.File2Base64Text(fileUpPathName);
		const formData = new FormData();

		const timestamp1 = new Date().getTime();
		const myfff = base64toBlob(base64file);

		const timestamp2 = new Date().getTime();

		//alert(myfff);
		formData.append("FileData", myfff, "myFileName");
		const aaa = '${pageContext.request.contextPath}';

		const strUrl = jsWebOffice.WebUrl + '/AJAXServer?OPTION=MultiPartUpload&FILENAME=' + filename;
	...
	this.WebSaveBase64 = function () {
		...
		const base64file = officeobj.File2Base64Text(fileUpPathName);
		//alert(base64file);
		const newBase64 = base64file.replace(/\+/g, "%2B");
		//alert(newBase64);
	...
	this.WebSave = function () {
		...
		if (this.WebSetMaxFileSize(mSavePath)) {
			...
			if (this.SAVEFILE(httpclient, mSavePath)) {
				let ISO = httpclient.GetResponseHeader("ISO");// 获取返回值
				//this.Close();
	...
	this.WebSaveAsHtml = function () {
		if (this.SaveAsHtml(this.getFilePath() + this.RecordID + ".html")) {
			let strDirPath = this.getFilePath() + this.RecordID + '.files'; //获取html files路径
			let FindFile = this.obj.FileSystem.FindToDirAsFileEx(strDirPath);//获取html文件夹里的内容
			let DirPath = FindFile.split("\r\n");
			let httpclient = this.obj.Http; // 设置http对象
			httpclient.Clear();
			this.WebSetMsgByName("RECORDID", this.RecordID);
	...
	this.WebSaveTemplate = function () {
		...
		this.WebSetMsgByName("TEMPLATE", this.Template);
		let mSaveResult = this.WebSaveLocalFile(this.getFilePath() + this.FileName);
		if (!(mSaveResult == 0)) {
			this.Status = "保存本地文档失败！错误代码为：" + mSaveResult;
			return false;
		}
	...
	this.WebUseTemplate = function () {
		...
		let mSaveResult = this.WebSaveLocalFile(this.getFilePath() + this.iWebOfficeTempName);//将当前文档保存下来
		if (!(mSaveResult == 0)) {
			this.Status = "保存本地文档失败！错误代码为：" + mSaveResult;
			return false;
		}
	...
	this.WebSaveImage = function (ImageType, ImageName) {
		if (this.FileType == ".doc" || this.FileType == ".docx" || this.FileType == ".wps") {
			let iFilePath = this.getFilePath() + this.RecordID + ".htm";
			let strDirPath = this.getFilePath() + this.RecordID + '.files'; //获取html files路径
			...
			let DirPath = new VBArray(this.obj.FileSystem.FindToDirAsFile(strDirPath)).toArray(); //获取html文件夹里的内容
			let httpclient = this.obj.Http; 							// 设置http对象
			httpclient.Clear();
			this.WebSetMsgByName("RECORDID", this.RecordID);
	...
	this.WebSaveBookMarks = function () {
		let httpclient = this.obj.Http;
		httpclient.Clear();
		let BKCount = this.obj.ActiveDocument.Bookmarks.Count; //获取书签数量
		for (let i = 1; i <= BKCount; i++) {
			let BookName = this.obj.ActiveDocument.Bookmarks.Item(i).Name;
			let BookValue = this.obj.ActiveDocument.Bookmarks.Item(BookName).Range.text;
			...
			if (httpclient.Send()) {
				...
				try {
					for (let i in jsonObj) {
						let BookName = i;
						let BookValue = jsonObj[i];
						if (this.obj.ActiveDocument.BookMarks.Exists(BookName)) {
							let mBookRange = this.obj.ActiveDocument.Bookmarks.Item(BookName).Range;
	...
	this.WebOpenSignature = function () {
		let FunExt = this.obj.FuncExtModule;
	...
	this.WebCheckSignature = function () {
		...
		FunExt.FileType = this.FileType;
		FunExt.SetDocument(this.obj.ActiveDocument);
		let i = FunExt.WebCheckSignature();
	...
	this.CreateFile = function () {
		let docType = this.getDocType(this.FileType); 			// 获取文档类型
		switch (docType) {
	...
	this.WebSaveLocal = function () {
		...
		let ext = fileNamePath.substring(fileNamePath.length - 3, fileNamePath.length);
		if (fileNamePath == "") {
			return true;
		}
		try {
	...
	this.WebAddBookMarks = function (BMarksName, BMarksValue) {
		if (this.FileType == ".doc" || this.FileType == ".docx") {
			if (this.obj.ActiveDocument.ProtectionType == "-1") {
				if (!this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) // 判断是否存在该书签
				{
					let BMVLength = BMarksValue.length;
					...
					this.obj.ActiveDocument.Application.Selection.MoveLeft(Unit = 1,
						Count = BMVLength);
					let StartR = this.obj.ActiveDocument.Application.Selection.Start;
					let EndR = this.obj.ActiveDocument.Application.Selection.Start + BMVLength;
			...
			if (this.obj.ActiveDocument.ProtectionType == "-1") {
				try {
					if (this.obj.ActiveDocument.BookMarks.Exists(vbmName)) {				// 判断是否存在该书签
						let vRange = this.obj.ActiveDocument.Bookmarks.Item(vbmName).Range;
	...
	this.WebOpenPicture = function () {
		if (this.FileType == ".doc" || this.FileType == ".docx" || this.FileType == ".wps") {
			let spDlg = this.obj.ActiveDocument.Application.Dialogs.Item(163);
			spDlg.Show();
		} else if (this.FileType == ".xls" || this.FileType == ".xlsx" || this.FileType == ".et") {
			let spDlg = this.obj.ActiveDocument.Application.Dialogs.Item(342);
			spDlg.Show();
		}
	}
	...
	this.WebDownLoadFile = function (Url, FileName) {
		...
		let info = httpclient.Open(kwoHttpGet, Url, false);
		if (info) {
			let send = httpclient.Send();
			if (send) {
				if (httpclient.Status == 200) {
		...
		if (OfficeVersion != "11.0") {
			if ((this.FileType == ".doc") || (this.FileType == ".docx")
				...
				try {
					...
					this.Status = e.description;
					alert("保存PDF异常,可能Office没有保存PDF功能，建议安装SaveAsPDFandXPS插件");
					return false;
				}
			} else if ((this.FileType == ".xls") || (this.FileType == ".xlsx") || (this.FileType == ".et")) {
				try {
